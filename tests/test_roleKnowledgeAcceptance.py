...
def testConfirmedUnknownRolePersistsExpectedYaml(qtbot, tmp_path, fixturePath: Path) -> None:  # type: ignore[no-untyped-def]
    """Drive the real Qt role editor and compare its persisted YAML to confirmed golden data."""

    fixture = _fixtureLoad(fixturePath)
    if fixture["confirmationState"] != "confirmed":
        pytest.skip(f"{fixture['roleCode']} still requires confirmed Football Manager evidence")
